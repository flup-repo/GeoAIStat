...
|------|--------|----------------|-------------------|
| 2026-03-08 | self | Edited the napkin with a one-off Python script even though repo edits are supposed to go through `apply_patch`. | Use `apply_patch` for repo file changes, including operational notes like the napkin. |
| 2026-03-08 | self | Mutated `camera.position` from a `useThree()` return value and tripped the React hooks immutability lint rule. | In R3F scene rigs, mutate the frame callback `camera` argument inside `useFrame` instead of mutating a hook-returned object. |
| 2026-03-08 | self | Started from PLAN.md again before checking how much of the app was already implemented. | Re-read the current scene, pipeline, and verification status before treating plan gaps as untouched work. |
...
- `world-atlas` + `us-atlas` plus a build-time extraction script are a clean way to localize globe geometry without introducing a runtime network dependency.
...
- The globe now loads local country and US state geometry from generated `public/data/geometry/*.json` artifacts instead of fetching country wireframes at runtime.
